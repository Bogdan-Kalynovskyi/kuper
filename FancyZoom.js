...
function preloadingStop(){
			...
			if(imgPreload)imgPreload.onload = null;
...
function zoomIn() {
	...
	if(typeof e == 'object')cancelEvent(e);
...
function zoomElement() {
		...
		//// !!!!!!
		if(zoomCurrent==1)zoomDiv.style.visibility = "visible";
...
//###########################################		
	if (zoomCurrent >= tempSteps) {
		...
		if(time>CPU)setTimeout(execWhenDone, (time - CPU)*1.3);//todo test
		else execWhenDone();
...
function fadeout(e) {
	if(ie)	e.style.visibility = "hidden"
	else TINY.alpha.set(e,0,8,1);
}

function fadein(e) {
	e.style.visibility = "visible";
	if(ie)e.style.filter=''
	else TINY.alpha.set(e,100,10);
}
