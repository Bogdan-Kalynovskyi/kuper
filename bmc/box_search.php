...
<fieldset class="search_box" id="__s2">
	...
	<input type="text" name="<?php echo SEARCH_HASH; ?>" id="<?php echo SEARCH_HASH; ?>" value="Site search..." 
		onfocus="if(this.value=='Site search...')this.value='';document.getElementById('__s2').style.opacity=1;document.getElementById('__s3').style.display='block'" 
		onblur="if(this.value=='')this.value='Site search...';//document.getElementById('__s2').style.opacity=0.5" 
	...
	<div id="__s3"><!-- easing -->
		...
		<select name="where[]" size="1">
			<?php foreach($STA as $s){
					...
					if($tbl == $s) $temp=' selected="selected"';
					else 			 $temp='';
					if($s=='')	$s = 'All';//=)
