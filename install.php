...
<head>
    ...
    <script>//make it nicer
        ...
        !--function db_checker() {//todo
            ...
            var features = "width=" + width + ",height=" + height + ",screenX=" + screenX + ",left=" + screenX + ",screenY=" + screenY + ",top=" + screenY;
            features += ",+toolbar=No,menubar=No,resizable=No,scrollbars=No,status=No,location=No";
            var url = 'check_db_exists.php?install=true&amp;sid=<?php echo md5(sha1(session_id())); ?>' + '&amp;db_host=' + escape(document.install.db_host.value) + '&amp;db_name=' + escape(document.install.db_name.value) + '&amp;db_user=' + escape(document.install.db_user.value) + '&amp;db_pass=' + escape(document.install.db_pass.value) + '&amp;db_prefix=' + escape(document.install.db_prefix.value);
//check fields
        ...
        function isBad(sInString) {//empty or non alphanumeric
            if (sInString == null || sInString == undefined || sInString === '') {
                return true;
            }
            if (!(sInString.match(/\S/))) {
                return true;
            }
...
if (isset($_POST['db_prefix']) && ($_POST['db_prefix'] == '<auto>')) {
    $_POST['db_prefix'] = '';
}
foreach ($_POST as &$p) {
    $p = trim($p);
...
foreach ($_POST as $key => $el) {
    if ($key != 'c_url' && $key != 'admin_pass' && $key != 'admin_pass2') {
...
if (!isset($_POST['db_pass']) || strlen($_POST['db_pass']) < 5) {
    echo '<b><br><small style="color:#900">Using DB pasword shorter than 5 symbols is security risk!</small></b><br>...';
}
//<auto...>
...
$my_db = $_POST['db_name'];
$my_prefix = $_POST['db_prefix']; //$my_extra connection
...
if (!is_writable("vars/config.php") /* || !is_writable("vars/session.php")*/) {
    ...
    @chmod("vars/", 0600);
    @chmod("vars/config.php", 0600); /*!@chmod("vars/session.php", 0600)*/
    if (!is_writable("vars/config.php")) {
        @chmod("vars/", 0600);
        @chmod("vars/config.php", 0600);
    }
    if (!is_writable("vars/config.php")) /* phpbb chmod */ {
...
<div style="border:2px solid #c66;color:#000;padding:6px;text-align:left">
    <u>Note:</u><br> File <strong>install.php</strong> <?php if ($ren) { ?>renamed to <strong>_install.php**** </strong><?php } ?>. It's a security risk to leave it on your
    webserer. It's recommended to <a style="text-decoration:underline" href="install_delete.php?id=<?php echo $fname; ?>&amp;sid=<?php echo md5(sha1(session_id())); ?>"
                                     target=_blank>delete it by clicking this link</a>. If you ever need to reinstall the site, get install.php out of the installation archive and
