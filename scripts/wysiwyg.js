var WYSIWYG = {
	...
	"return": 		  ['Return', 			   'СТВОРИТИ НОВУ СТОРІНКУ', 'return.gif', 			  'return_on.gif'],
	...
	insertImage: function(src, width, height, align, border, alt, space, color, n, styleClass) {
		...
		if(styleClass != '') WYSIWYG_Core.setAttribute(lin, "classname", styleClass);
		
		style = '';
		if(width) style += "width:" + width + '; ';
		if(height) style += "height:" + height + '; ';
		if(align) style += "float:" + align + '; ';
		if(border) style += "border-width:" + border + 'px; ';
		if(color) style += "border-color:" + color + '; ';
		if(space) style += "margin:" + space + 'px; ';
//		if(space) style += "padding:" + space + 'px; ';
	...
	insertLink: function(href, target, style, styleClass, name, n, title) {
		...
		if(title != '') WYSIWYG_Core.setAttribute(lin, "title", title);
		if(styleClass != '') WYSIWYG_Core.setAttribute(lin, "class", styleClass);
		if(styleClass != '') WYSIWYG_Core.setAttribute(lin, "classname", styleClass);
	...
	updateStatusBar: function(n) {
		...
		if(nodeTree == null || nodeTree == undefined) { return; }
		// format the output
		var outputTree = "";
...
window.open(pagePopupFile + '?wysiwyg=' + n+'&blog='+document.modpost.blog.value+'&id='+document.modpost.id.value, 'popup0', 'location=0,status=0,menubar=0,toolbar=0,scrollbars=0,resizable=0,width=350px,height=100px,top=' + popupPosition.top + ',left=' + popupPosition.left).focus();			
	...
	insertNodeAtSelection: function(insertNode, n, range, sel) {
		...
		if(sel == null)var sel = this.getSelection(n);
		
		// get the first range of the selection
		// (there's almost always only one range)
		if(range == null)var range  = sel.getRangeAt(0);
	...
	findParent: function(parentTagName, range){
		...
		try {
			if(!WYSIWYG_Core.isMSIE) {
				...
				if(elmWorking != null && elmWorking != undefined) return elmWorking;
