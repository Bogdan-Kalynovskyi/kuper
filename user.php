...
if(@$_POST[FORM_HASH]==3){//pin();
	...
	foreach($_POST['d'] as $k=>$v){
		if(isnumeric($k) && $v){
			if($k == 1){echo('<h1>Ñòàðòîâóþ ñòðàíèöó íåëüçÿ óäàëèòü</h1>');continue;}
			unset($_POST['d'][$k]);
			unset($_POST['v'][$k]);
			unset($_POST['f'][$k]);
...
function pin(){};//todo class redirector	1ñòîð11 âåðñûé ðåäàãóâàííÿ, çðó÷íû û äðóæíû ðåäûðåíòè, ïîêàçóâàííÿ îñòàííüî1 çðîáëåíîð1 âêðñ11 !!. õî÷ó ðåäèðåêòèòèñü òóäà!
/*		if(isset($_SERVER['HTTP_REFERER']) && !isset($_COOKIE['BMC_redirect']) && ! isset($_REQUEST['nest']) 
		 && basename($_SERVER['HTTP_REFERER']) !== basename($_SERVER['SCRIPT_NAME']))
			bmc_Go(-1);/*õòî òàêó õóéíþ íàïèñàâ?*/
