...
function my_find_path(&$source_name, $upload_dir){//я не задоволений ц1эю процедурою
		...
		my_chop(A_ROOT.'/', $source_name);$source_name=trim($source_name);			
...
/*	do{
	...
	foreach($valid_upload_folders as $s){
	 		if(strpos($source_name, $s) === 0
	 		|| strpos($source_name, '/'.$s) === 0
	 		|| strpos($source_name, './'.$s) === 0){
		 			$flag = true;
		 			break;
	 		}
...
////////////////////////////////////////////////////////////////////////////////////////
	if( noempty( $_FILES[ $name ]['type']) )
	{
		...
		if(isempty($_FILES[$name]['name']))return null;
		if(isempty($_FILES[$name]['tmp_name']))return null;
		if($_FILES[$name]['error'])return null;
		//-content-type
		if ($_FILES[ $name ]['size'] > $max_pic_size) return null;

		$source_name = valid_filename($_FILES[$name]['name']);
		if(get_magic_quotes_gpc())$source_name = stripslashes($source_name);
...
////////////////////////////////////////////////////////////////////////////////////////	
	elseif(noempty($_POST[$name]))
	{
		...
		if(substr($source_name,0,4)=='www.')$source_name = 'http://'.$source_name;
		
		my_replace_url($source_name);


	 	if((strtolower(substr($source_name,0,7)=='http://'))||
	 	strtolower(substr($source_name,0,8)=='https://')||
	 	strtolower(substr($source_name,0,6)=='ftp://')){
	 		
			 		if(!url_exists($source_name))return null;
					$local_url = false;
		 	
	 	}else{
	 		
	 				if(! my_find_path ($source_name, $upload_dir)) return null;
		 			if(! is_file(A_ROOT./*rawurldecode*/($source_name)))return null;
		 			$local_url = true;
	...
	if(isempty($source_name)) return '';	//ATTENTION, sperial return value!
	...
	if(!in_array($ext, $supportedextentions)) return null;
    ...
    if($from_url){
		...
		if(noempty($new_name)){
			...
			if(is_file($f2))
   			...
   			if(!move_uploaded_file($_FILES[ $name ]['tmp_name'], $f2)) return null;
...
//source_name
	...
	if(isempty($source_name)) return '';	



	$ext = strtolower(substr($source_name, strrpos($source_name, '.')));//strrchr ??
	if(in_array($ext, $UNsupportedextentions)) return null;
