...
function recursive_hidden($err)
{
	foreach($err as $key => $p){
		...
		elseif(substr($key, 0, 3) != 'wp_')
			echo '<input type="hidden" name="'.quotes($key).'" value="'.quotes($p).'" />';//expires 
...
function log_bad_ip($NOT_silent=false, $login_name='') {
		global $db, /*$user_message,*/ $do_cap;

//////////////////////////////////////////////////////////
////////////////////////////////////////////

		
		if(isset($do_cap) && $do_cap)return true;
...
<p>
	...
	<input type="text" name="wp_keystring" id="keystring" class="input" value="" size="20" tabindex="50" />
	</label>
</p>';
} 
