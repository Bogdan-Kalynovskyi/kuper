	...
	$fon = htmlspecialchars(rawurldecode($ph['fon']));
		...
		<div class="baba" id="_$k">
			...
			<label><span>
				...
				<a href="index.php?id=$key" target="_blank" class="_eye_" title="просмотреть"><img src="img/eye_small.gif" alt="&bull;"/></a>Заголовок</span> 
...
?>
	...
	<img src="images/plus.gif" title="добавить" alt="добавить" onclick="add()" style="margin-left:30px; cursor:pointer" />
	<br/>
	<br/>
...
<script type="text/javascript">
		...
		function del(i){
		//todo test ie	
			if(confirm('Уничтожить эту фотографию?')){
				var element = $('_'+i);
...
'	<div class="baba" id="_'+n+'">			<label><span><a class="_eye_" title="пока нечего смотреть"><img src="img/eye_small.gif" alt="&bull;"/></a>Заголовок</span> 		<input type="text" name="v['+nn+']" id="v'+n+'" class="title" />	<b>				 <a href="#" title="вверх" onclick="up('+n+'); return false"><img src="img/up.png" alt="вверх" /></a> 	 <a href="#" title="вниз" onclick="down('+n+'); return false"><img src="img/down.png" alt="вниз" /></a>				 <a href="#" title="стереть" onclick="del('+n+'); return false"><img src="img/del.png" alt="стереть" /></a>			</b>			</label>			<label><span>Картина</span>			<img src="" alt="нет" id="_i'+n+'" /> 	URL<input type="text" name="i['+nn+']" id="i'+n+'" /> или файл<input type="file" id="__i'+n+'" name="i'+nn+'" /> &nbsp;<a onclick="clrnpt(\'i'+n+'\');return false">Убрать</a>		</label>			<label><span>Фон</span>			<img src="" alt="нет" id="_f'+n+'"/> 		URL<input type="text" name="f['+nn+']" id="f'+n+'" /> или файл<input id="__f'+n+'" type="file" name="f'+nn+'" />	&nbsp;<a onclick="clrnpt(\'f'+n+'\');return false">Убрать</a>	</label>			<label> 			<span style="float:left">Описание</span>	 		<textarea name="t['+nn+']" id="t'+n+'"></textarea></label>		</div><hr/>';
