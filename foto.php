<?php
	...
	function back_link_func(){
		echo '?page='.BLOG; if($sex) echo '&go='.$one['switch'];
	}


	$sql=$sex='';$bmc_vars['blog_y']+=20;
...
///////// 3	
	elseif(isnumeric($_GET['id'])){
		...
		if(!$data)die('<h1><br/><br/><br/>������� �����</h1>'.backbutton());
...
.to_bottom{opacity:0.7;text-decoration:none;
clear:both;font-size:12px;color:#33a;left:22px;
}
.to_bottom:hover{opacity:1;text-decoration:underline;
	color:#c00;
}
