...
<p id="nav">
<a href="./?_account_=register" title="Become a member" tabindex="120">Register</a> | <a href="./?_account_=email_request" title="Password Lost and Found" tabindex="130">Lost your password?</a></p>
...
<script>
try{document.getElementById('<?php echo $focused_field ?>').focus();}catch(e){}
var js_on = true;
...
function validate() {
	...
	try{
		...
		if(f1.value === '') {
			f1.focus();err[0] = "Please enter  login";
		}
		else if(f1.value.length < 3) {
			f1.focus();err[0] = "Login too short, 3 letters  minimum";
		}

		if(f2.value === '') {
			f2.focus();err[1] = "Please enter  password";
		}
		else if(f2.value.length < 3) {
			f2.focus();err[1] = "Password too short, 3 letters  minimum";
		}
