		...
		switch($el){
			...
			case 'picture':
				if(isset($_FILES[$key])&&(!$_FILES[$key]['error'])){
					include_once A_HOME."upload_pic.php";$x=$bmc_vars['big_x'];$y=$bmc_vars['big_y'];
...
////////////////////		
	if(($el=='picture' || $el=='file')&& isset($_POST[$key.'__no'])){
			...
			if($fl){ unlink( A_ROOT.htmlspecialchars_decode($fl ));}
	}
//////////////////
...
global $db;
	...
	foreach($required as $r){
		switch($fields[$r]){
			...
			case 'integer':
				if(!isnumeric($_POST[$r])) error_msg("Please insert a number - <b>$r</b>: {$_POST[$r]}");
				break;
				
			case 'login': 
			    if(preg_match("/[^a-zA-Z0-9_\-\s]/", $_POST[$r]) || strlen($_POST[$r]) < 3) error_msg("Login should contain alphanumeric characters only and be minimum 2 symbols long - <b>$r</b>: {$_POST[$r]}");
			    break;

			case 'textarea':
				if(!isset($_POST[$r]) || empty($_POST[$r])) error_msg("Please fill the field - <b>$r</b>: {$_POST[$r]}");
				break;
				
			case 'password': 
				if(strlen($_POST[$r])< 6 )	error_msg('Too short password, 6 chars mimimum');
				if($_POST[$r] != $_POST['pass1'])	error_msg('Passwords do not match');
				break;
				
			default:
				if(isempty($_POST[$r])) error_msg("This field is required - <b>$r</b>: {$_POST[$r]}");
		}
	}
