var WYSIWYG = {
    ...
    Settings: function () {
        ...
        // CSS Directory File
        this.CSSFile = "scripts/wysiwyg.css";
        ...
        this.Toolbar[1] = new Array("delete", "seperator", "subscript", "superscript", "seperator", "cut", "copy", "paste", "removeformat", "seperator", "undo", "redo", "seperator", "inserttable", "createlink", "save", "insertimage", "slideshow", "seperator", "preview", "print", "seperator", "viewSource", "seperator", "return");
        ...
        this.DropDowns['font'] = {
            ...
            width: "90px",
            elements: new Array("Arial", "Sans Serif", "Tahoma", "Verdana", "Courier New", "Georgia", "Times New Roman", "Impact", "Comic Sans MS", "Courier", "Courier New")
        };
        // Font sizes
    ...
    ToolbarList: {
        ...
        "bold": ['Bold', 'Жирний', 'bold.gif', 'bold_on.gif'],
        "italic": ['Italic', 'Курсив', 'italics.gif', 'italics_on.gif'],
        "underline": ['Underline', 'Підкресдений', 'underline.gif', 'underline_on.gif'],
        "strikethrough": ['Strikethrough', 'Закреслений', 'strikethrough.gif', 'strikethrough_on.gif'],
        "seperator": ['', '', 'seperator.gif', 'seperator.gif'],
        "subscript": ['Subscript', 'Внизу', 'subscript.gif', 'subscript_on.gif'],
        "superscript": ['Superscript', 'Вгорi', 'superscript.gif', 'superscript_on.gif'],
        "justifyleft": ['Justifyleft', 'Текст зліва', 'justify_left.gif', 'justify_left_on.gif'],
        "justifycenter": ['Justifycenter', 'Текст в центрі', 'justify_center.gif', 'justify_center_on.gif'],
        "justifyright": ['Justifyright', 'Текст справа', 'justify_right.gif', 'justify_right_on.gif'],
        "justifyfull": ['Justifyfull', 'Текст по ширині', 'justify_justify.gif', 'justify_justify_on.gif'],
        "unorderedlist": ['InsertUnorderedList', 'НЕнумерований список', 'list_unordered.gif', 'list_unordered_on.gif'],
        "orderedlist": ['InsertOrderedList', 'Нумерований список', 'list_ordered.gif', 'list_ordered_on.gif'],
        "outdent": ['Outdent', 'Параграф вліво', 'indent_left.gif', 'indent_left_on.gif'],
        "indent": ['Indent', 'Параграф вправо', 'indent_right.gif', 'indent_right_on.gif'],
        "cut": ['Cut', 'Вирізати', 'cut.gif', 'cut_on.gif'],
        "copy": ['Copy', 'Скопіювати', 'copy.gif', 'copy_on.gif'],
        "paste": ['Paste', 'Вставити', 'paste.gif', 'paste_on.gif'],
        "forecolor": ['ForeColor', 'Колір тексту', 'forecolor.gif', 'forecolor_on.gif'],
        "backcolor": ['BackColor', 'Колір фону', 'backcolor.gif', 'backcolor_on.gif'],
        "undo": ['Undo', 'Undo', 'undo.gif', 'undo_on.gif'],
        "redo": ['Redo', 'Redo', 'redo.gif', 'redo_on.gif'],
        "inserttable": ['InsertTable', 'Вставити Таблицю', 'insert_table.gif', 'insert_table_on.gif'],
        "insertimage": ['InsertImage', 'Вставити Малюнок', 'insert_picture.gif', 'insert_picture_on.gif'],
        "createlink": ['CreateLink', 'Створити гіперссилку', 'insert_hyperlink.gif', 'insert_hyperlink_on.gif'],
        "viewSource": ['ViewSource', 'Переглянути/редагувати HTML код', 'view_source.gif', 'view_source_on.gif'],
        "viewText": ['ViewText', 'Назад в редактор', 'view_text.gif', 'view_text_on.gif'],
        "fonts": ['Fonts', 'Шрифт', 'select_font.gif', 'select_font_on.gif'],
        "fontsizes": ['Fontsizes', 'Розмір шрифта', 'select_size.gif', 'select_size_on.gif'],
        "headings": ['Headings', 'Заголовки', 'select_heading.gif', 'select_heading_on.gif'],
        "preview": ['Preview', 'Швидкий попередній перегляд', 'preview.gif', 'preview_on.gif'],
        "print": ['Print', 'Надрукувати', 'print.gif', 'print_on.gif'],
        "removeformat": ['RemoveFormat', 'очистити MS Word HTML', 'remove_format.gif', 'remove_format_on.gif'],
        "delete": ['Delete', 'Видалити елемент', 'delete.gif', 'delete_on.gif'],
        "save": ['Save', 'Додати файл для скачування', 'save.gif', 'save_on.gif'],
        "return": ['Return', 'СТВОР�?Т�? НОВУ СТОРІНКУ', 'return.gif', 'return_on.gif'],
        "slideshow": ['Slideshow', 'Слайдшоу', 'loading.gif', 'loading.gif']
    },
     ...
     */
    /*RemoveAllFormat: function(n) {
     var doc = this.getEditorWindow(n).document;
     // get selection and range
     var sel = this.getSelection(n);
     var range = this.getRange(sel);

     var x = range.toString();

     WYSIWYG.InsertHTML(x,n);
     }*/

    /*.* дополнительные нерпечатаемые символы */

    insertImage: function (src, width, height, align, border, alt, space, color, n, styleClass) {
        ...
        var update = (img == null || img == undefined) ? false : true;
        if (!update) {
        ...
        WYSIWYG_Core.setAttribute(img, "title", alt);
...
//		if(space) style += "padding:" + space + 'px; ';
        ...
        if (WYSIWYG_Core.isMSIE) {
            ...
            this.insertNodeAtSelection(img, n, range, sel);
        }
    },
     ...
     */
    insertLink: function (href, target, style, styleClass, name, n, title) {
        ...
        var update = (lin == null || lin == undefined) ? false : true;
        if (!update) {
        ...
        if (title != '') {
            WYSIWYG_Core.setAttribute(lin, "title", title);
        }
        if (styleClass != '') {
            WYSIWYG_Core.setAttribute(lin, "class", styleClass);
        }
        ...
        if (WYSIWYG_Core.isMSIE) {
            ...
            this.insertNodeAtSelection(lin, n, range, sel);
        }
    },
    ...
    insertP: function (n) {
        ...
        // find a as parent element
        lin = this.findParent("p", range);

        // check if parent is found
        var update = (lin == null || lin == undefined) ? false : true;
        if (!update) {
            lin = doc.createElement("p");
        }
        // on update exit here
        if (update) {
            return;
    ...
    getNodeTree: function (n) {
        ...
        while (node != null && node != undefined && node.nodeName != "#document") {
            nodeTree[ii] = node;
     ...
     * @param {String} n The editor identifier (the textarea's ID)
     */
    emptySelection: function (range) {
        return range.collapsed || (this.isMSIE && !range.boundingWidth);
    },
    ...
    execCommand: function (n, cmd, value) {
        ...
        var filePopupFile = this.config[n].PopupsDir + 'insert_image.html';
        var pagePopupFile = this.config[n].PopupsDir + 'create_page.php';
...
//		if(typeof this.config[n].ImagePopupFile != "undefined" && this.config[n].ImagePopupFile != "") {
        ...
        filePopupFile = this.config[n].FilePopupFile;
        slidePopupFile = this.config[n].SlidePopupFile;
...
//		}
        ...
        switch (cmd) {
            ...
            case "ForeColor":
                ...
                window.open(this.config[n].PopupsDir + 'select_color.html?color=' + currentColor + '&command=' + cmd + '&wysiwyg=' + n, 'popup1', 'location=0,menubar=0,toolbar=0,status=0,scrollbars=0,width=210,height=165,top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
                break;

            // BackColor
            case "BackColor":
                var currentColor = toHexColor(this.getEditorWindow(n).document.queryCommandValue(cmd));
                window.open(this.config[n].PopupsDir + 'select_color.html?color=' + currentColor + '&command=' + cmd + '&wysiwyg=' + n, 'popup2', 'location=0,menubar=0,toolbar=0,status=0,scrollbars=0,width=210,height=165,top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
                break;

            // InsertImage
            case "InsertImage":
                window.open(imagePopupFile + '?wysiwyg=' + n, 'popup3', 'location=0,status=0,menubar=0,toolbar=0,scrollbars=0,resizable=0,toolbar=0,width=' + (imagePopupWidth + 105) + ',height=' + (imagePopupHeight + 75) + ',top=' + (popupPosition.top - 20) + ',left=' + (popupPosition.left - 110)).focus();
                break;
            ...
            // Create Link
            case "CreateLink":
                window.open(this.config[n].PopupsDir + 'insert_hyperlink.html?wysiwyg=' + n, 'popup4', 'location=0,status=0,menubar=0,toolbar=0,scrollbars=0,resizable=0,width=350,height=168,top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
                break;

            // InsertTable
            case "InsertTable":
                window.open(this.config[n].PopupsDir + 'create_table.html?wysiwyg=' + n, 'popup5', 'location=0,status=0,menubar=0,toolbar=0,scrollbars=0,resizable=0,width=500,height=222,top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
                break;
            ...
            case "Preview":
                window.open(this.config[n].PopupsDir + 'preview.html?wysiwyg=' + n, 'popup7', 'location=0,status=0,menubar=0,toolbar=0,scrollbars=1,resizable=1,width=' + this.config[n].PreviewWidth + ',height=' + this.config[n].PreviewHeight + ',top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
                break;
            ...
            // Save
            case "Save":
                window.open(filePopupFile + '?wysiwyg=' + n, 'popup8', 'location=0,status=0,menubar=0,toolbar=0,scrollbars=0,resizable=0,width=' + (imagePopupWidth + 20) + ',height=' + (imagePopupHeight - 38) + ',top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
                break;

            case "Slideshow":
                window.open(slidePopupFile + '?wysiwyg=' + n, 'popup9', 'location=0,status=0,menubar=0,toolbar=0,scrollbars=1,resizable=1,width=920, height=700,top=50, left=50').focus();
                break;

            // Return
            case "Return":
                window.open(pagePopupFile + '?wysiwyg=' + n + '&blog=' + document.modpost.blog.value + '&id=' + document.modpost.id.value, 'popup0', 'location=0,status=0,menubar=0,toolbar=0,scrollbars=0,resizable=0,width=350px,height=100px,top=' + popupPosition.top + ',left=' + popupPosition.left).focus();
                break;
    ...
    /* ---------------------------------------------------------------------- *\
     ...
     \* ---------------------------------------------------------------------- */
    insertNodeAtSelection: function (insertNode, n, range, sel) {
    ...
    findParent: function (parentTagName, range) {
        ...
        try {
            if (!WYSIWYG_Core.isMSIE) {
                ...
                if (elmWorking != null && elmWorking != undefined) {
                    return elmWorking;
                }
...
var WYSIWYG_Table = {
    ...
    create: function (n, tbl) {
        ...
        // check if parent is found
        var update = (table == null || table == undefined) ? false : true;
        if (!update) {
            table = tbl;
        }
        ...
        if (WYSIWYG_Core.isMSIE) {
            ...
            WYSIWYG.insertNodeAtSelection(table, n, range, sel);
        }
    ...
    _enableHighlighting: function (node) {
        ...
        //WYSIWYG_Core.setAttribute(node, "style", "border:1px dashed #AAAAAA;");//bodya
    },
...
full.FilePopupFile = "popups/insert_file.php";
full.SlidePopupFile = "popups/slideshow.php";
full.ImagePopupWidth = 600;
full.ImagePopupHeight = 245;

var full1 = new WYSIWYG.Settings();
full1.ImagesDir = "images/";
full1.PopupsDir = "popups/";
full.CSSFile = "scripts/wysiwyg.css";
full1.Width = "99.9%";
full1.Height = "190px";
...
full1.FilePopupFile = "popups/insert_file.php";
full1.SlidePopupFile = "popups/slideshow.php";
full1.ImagePopupWidth = 600;
full1.ImagePopupHeight = 245;
