...
<style> input{font-size:16px;width:160px;margin:3px 0 3px 20px;float:none}
 .form_fields{font-size:14px;width:320px;border-width:3px;margin-top:120px}
 input[type=checkbox]{width:15px;height:15px;vertical-align:middle; margin:6px 153px 6px 20px;
 	/* *margin-right:100px;*/
 padding:0}
</style>
...
<?php if(isset($user_message)) echo '<span class="bold_red">'.$user_message.'</span>'; ?><br/><br/>
...
<script>
<!--
try{ document.my_form.user_login.focus(); }catch(e){}


	function validate() {

		if(document.my_form.user_login.value.length < 3) {
			document.my_form.user_login.focus();alert("Short login, 3 chars minimum");
			 return false;
		}
		if(document.my_form.password.value.length < 3) {
			document.my_form.password.focus();alert("Short password, 3 chars minimum");
			return false;
		}
		try{
