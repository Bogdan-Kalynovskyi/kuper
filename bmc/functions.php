...
// RETURN USERNAME AND WRITE SESSION EXPIRED
        ...
        if (($time - $res['time']) > LOGIN_TIME_LIMIT || ($res['ip'] != $ip)) {
        ...
        if (($time - $res['time']) > 1060 || (isset($_COOKIE['BMC_time']) && ($time - $_COOKIE['BMC_time'] > 60))) {
...
function wasEdited (&$post) {
    return (isset($post['edited_by']) && ($post['edited_by'] != 0) && ($post['edited_by'] != $post['author']));
