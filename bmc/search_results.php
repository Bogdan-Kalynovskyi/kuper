...
if(noempty($_POST[SEARCH_HASH])){
		...
		{
		global $db;	if(!$table)return;//all?
		
		
							if($table=='mail')	$sq0='`atname` AS ';//poisk ne tolko po name|desc no i attach
							else $sq0='';
							
							if($table=='mail' && LEVEL < 3)	$sq1=' AND	user='.a($USER['id']);
							else $sq1='';
				...
				if($table!='mail'){
					$res = $db->query("SELECT id, `name`, `desc` FROM `".PRF."$table` 
						WHERE `name` LIKE '%$x%' ORDER BY `date` DESC LIMIT 10");//pageview here
				}else{
				 	$res = $db->query("SELECT u.id, u.$sq0`name`, u.`desc` FROM `".PRF."$table` u INNER JOIN `".PRF."bids` b  ON  1 $sq1
...
//----------------------------------------------------------------------------------------- 
	...
	<h1>Search</h1>';
	if(!isset($_POST['where']) || !is_array($_POST['where'])) $_POST['where'] = $table;
