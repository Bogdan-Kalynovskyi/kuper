...
var zoomOpen = false, zoomRunning = false, zoomActive = null, preloadTime = 0, 			//time from start preloading. used to kill hanging preloads
...
//###########################################		
    if (zoomCurrent >= tempSteps) {
        ...
        if (time > CPU) {
            setTimeout(execWhenDone, (time - CPU) * 1.2);
        }//todo test
        else {
            execWhenDone();
        }
...
function findElementPos(elemFind) {// ie?
    var elemX = 0;
    var elemY = 0;
    var fixed = false;
