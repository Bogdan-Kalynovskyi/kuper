...
?>	
    ...
    <div id="login">
	    <div id="cap-top"></div>
	    <div id="cap-body">
	    <div id="branding"><h1 style="margin:-9px 0 21px 40px;color:#dfdfdf;font-size:32px;">Oldo Admin Tool</h1><br/><?php if(isset($user_message)) echo '<div style="margin:-24px 0 4px 44px; color:#F55; font-weight:bold">'.$user_message.'</div>'; ?></div>		
            <div id="panelLogin">
	...
	function validate() {
		...
		if(document.getElementById('txtLogin').value.length < 3) {
			document.getElementById('txtLogin').focus();alert("Short login, 3 chars minimum");
			 return false;
		}
		if(document.getElementById('txtPassword').value.length < 3) {
			document.getElementById('txtPassword').focus();alert("Short password, 3 chars minimum");
			return false;
		}
		return true;
	}
