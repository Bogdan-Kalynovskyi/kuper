...
CSSParser.arrayContainsEach = function (haystack_ary, needles_ary) {
    ...
    for (n = 0; n < nlen; n++) {
        match = false;
        for (h = 0; h < hlen; h++) {
            if (haystack_ary[h] == needles_ary[n]) {
                match = true;
            }
        }
        if (!match) {
...
CSSStyleGroup.prototype.match = function (elem, pseudo) {
    var i, len = this.selectors.length;
    for (i = 0; i < len; i++) {
        if (this.selectors[i].match(elem, pseudo)) {
            return this.selectors[i].properties;
        }
    }
    return false;
}
...
CSSSelectorDefinition.prototype.match = function (element, pseudo) {
    ...
    if (!selector.match(compare_elem, pseudo)) {
        return false;
    }
    while (compare_elem && index >= 0) {
        ...
        if (pseudo) {
            pseudo = null;
        } // only used when compare_elem == element
    }
    return false;
}
...
CSSSingleSelector.prototype.parseData = function (selector_str) {
    ...
    if (selector_str) {
        this.type = selector_str.toLowerCase();
    }
}
...
CSSSingleSelector.prototype.match = function (element, pseudo) {
    ...
    if (this.type != "*" && element.tagName.toLowerCase() != this.type) {
        return false;
    }
    if (this.id != "*" && element.getAttribute("id") != this.id) {
        return false;
    }
    if (pseudo && pseudo != this.pseudoclass) {
        return false;
    }
    if (this.classes != "*" && !CSSParser.arrayContainsEach(CSSParser.getElementAttributeList(element, "class"), this.classes)) {
        return false;
    }
    if (this.attributes != "*" && !this.attributes.match(element)) {
        return false;
    }
    return true;
}
...
CSSAttributeSelector.prototype.match = function (elem) {
    if (this.attributeNames.length && !this.matchNames(elem)) {
        return false;
    }
    if (this.attributeValues.length && !this.matchValues(elem)) {
        return false;
    }
    if (this.attributeContains.length && !this.matchContains(elem)) {
        return false;
    }
    if (this.attributeBegins.length && !this.matchBegins(elem)) {
        return false;
    }
    return true;
}
CSSAttributeSelector.prototype.matchNames = function (elem) {
    ...
    for (i = 0; i < len; i++) {
        if (elem.getAttribute(this.attributeNames[i].name) == undefined) {
            return false;
        }
    }
    return true;
...
CSSAttributeSelector.prototype.matchValues = function (elem) {
    ...
    for (i = 0; i < len; i++) {
        ...
        if (elem.getAttribute(this.attributeValues[i].name) != this.attributeValues[i].value) {
            return false;
        }
    }
    return true;
