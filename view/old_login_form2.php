...
<form accept-charset="<?php echo $CHRST ?>" name="my_form" id="loginform" method="post" action="<?php echo $MY_URL.'/login.php';/*login*/?>" onsubmit="return validate();">
	<?php if(isset($user_message)) echo '<p class="bold_red">'.$user_message.'</p>'; ?><br/>
	<p>
		<label>Username<br />
	...
	<p class="forgetmenot"><label id="remember_label"><input name="wp_remember" type="checkbox" id="rememberme" value="1" title="For a month" tabindex="90" /><big> Remember Me</big></label></p>
	<p class="submit">
...
try{document.getElementById('user_login').focus();}catch(e){}
	...
	function validate() {
		...
		if(document.my_form.wp_user_login.value.length < 3) {
			document.my_form.wp_user_login.focus();alert("Short login, 3 chars minimum");
			 return false;
		}
		if(document.my_form.wp_user_pass.value.length < 3) {
			document.my_form.wp_user_pass.focus();alert("Short password, 3 chars minimum");
			return false;
		}
