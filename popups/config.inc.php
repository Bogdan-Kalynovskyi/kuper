...
if((substr($imagebaseurl, -1, 1)!='/') && $imagebaseurl!='') $imagebaseurl .=  '/';
if((substr($imagebasedir, -1, 1)!='/') && $imagebasedir!='') $imagebasedir .=  '/';
...
if(isset($_REQUEST['dir'])){$_GET['dir'] = $_POST['dir'] ? $_POST['dir'] : $_GET['dir'];
...
if($_GET['dir']) {
	...
	$dirok = true;
	$dirnames = explode('/', $_GET['dir']); $dotdotdir='';
...
global $supportedextentions, $leadon, $errormsg, $f;
	...
	if(!isset($_FILES[$f]) || isempty($_FILES[$f]['name']) || isempty($_FILES[$f]['tmp_name']) || ($_FILES[$f]['error'])){
		if($_FILES[$f]['error']) $errormsg = "Помилка завантаження";
		return null;
	}
