...
////------------------------------------------------------------------------------------------////
		if($_GET['_account_']=='restore_pass'){
				...
				if(bad_cap(true)) error_page($user_message);//user_message is a global variable taken from bad_cap
	
				$_POST['email'] = strtolower($_POST['email']);

				if(!isemail($_POST['email'])) error_page('Incorrect Email - <big>'.htmlspecialchars($_POST['email']).'</big>');

				$usr = $db->query("SELECT id,`login` FROM `".PRF."users` WHERE `email`=".a($_POST['email']), false );
				
				if(!$usr && $bmc_vars['email']!=$_POST['email']) error_page('You have entered a wrong email - <big>'.htmlspecialchars($_POST['email']).'</big>');
	...
	?>
		...
		<br/>
				...
				<?php show_ms(); ?>
					...
					<label>
						Your email<br />
						<input type="text" name="email" id="email" class="input" size="20" tabindex="10" />
					</label>
					</p>
